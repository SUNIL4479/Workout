...
import { DETAILED_EXERCISE_DATABASE, DetailedExercise, getExercisesByBodyFocus } from "./exerciseService";
...
export class WorkoutService {
  ...
  /**
   * Generates a safe, personalized workout plan with deduplication.
   * No exercise appears in more than one phase (warm-up / main / cool-down).
   */
  static generatePersonalizedWorkout(
    ...
    const warmUpEx = safeExercises.find((e) => e.category === "Mobility" && !usedIds.has(e.id))
      || safeExercises.find((e) => !usedIds.has(e.id))
      || safeExercises[0];
    ...
    const coolDownEx = safeExercises.find((e) => e.category === "Mobility" && !usedIds.has(e.id))
      || safeExercises.find((e) => !usedIds.has(e.id))
      || safeExercises[0];
  ...
  /**
   * Generates a body-part focused workout with robust deduplication.
   * No exercise ID repeats across phases, and animation types are diversified.
   */
  static generateBodyPartWorkout(
    ...
    const allMatching = getExercisesByBodyFocus(bodyPart as any);
